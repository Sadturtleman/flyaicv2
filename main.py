...
def get_detailed_gaze(landmarks):
    ...
    if pitch < -0.08:
        ...
        if avg_v_ratio < 0.42:
            final_v = "Up"
        elif avg_v_ratio > 0.56:
            final_v = "Down"

    res_parts = []
    if final_h:
        res_parts.append(final_h)
    if final_v:
        res_parts.append(final_v)
...
# ==================== 핵심: beforeme 감정 + main 나이/성별 통합 ====================

def background_analysis(crop_img, idx):
    """
    요구사항:
    - 감정(emotion): beforeme.py 파이프라인 (detector_backend='opencv', smoothed_emotion 사용)
    - 나이/성별(age/gender): main.py 파이프라인 (resize+blur+BGR->RGB, detector_backend='skip', EMA)
    """
    global faces_status
    try:
        ...
        # ---------- (1) EMOTION: beforeme.py 그대로 ----------
        # beforeme.py와 동일하게 crop_img + detector_backend='opencv'
        emo_result = DeepFace.analyze(
            crop_img,
            actions=['age', 'gender', 'emotion'],  # beforeme.py와 동일 구성 (감정 출력 동일성 유지)
            enforce_detection=False,
            silent=True,
            detector_backend='opencv'
        )[0]

        raw_emotion = emo_result.get('emotion', {}) if isinstance(emo_result, dict) else {}
        if not isinstance(raw_emotion, dict):
            raw_emotion = {}

        smoothed_emotion, smoothed_conf = get_smoothed_emotion(idx, raw_emotion)

        merged['emotion'] = raw_emotion
        merged['dominant_emotion'] = emo_result.get('dominant_emotion', merged.get('dominant_emotion', 'neutral'))
        merged['smoothed_emotion'] = smoothed_emotion
        merged['smoothed_confidence'] = smoothed_conf

        # ---------- (2) AGE/GENDER: main.py 그대로 ----------
        face_bgr = cv2.resize(crop_img, (224, 224), interpolation=cv2.INTER_LINEAR)
        face_bgr = cv2.GaussianBlur(face_bgr, (3, 3), 0)
        face_rgb = cv2.cvtColor(face_bgr, cv2.COLOR_BGR2RGB)

        ag_result = DeepFace.analyze(
            face_rgb,
            actions=['age', 'gender'],
        ...
        # AGE: -4 보정 + clamp + EMA
        if isinstance(ag_result, dict) and ('age' in ag_result):
            age_cal = float(ag_result['age']) - 4
            age_cal = max(5, min(80, age_cal))
            if isinstance(prev.get('age'), (int, float)):
                age_cal = (1 - alpha) * float(prev['age']) + alpha * age_cal
            merged['age'] = int(round(age_cal))
        else:
            merged.setdefault('age', prev.get('age', 0))

        # GENDER: dict EMA
        gender_dict = ag_result.get('gender') if isinstance(ag_result, dict) else None
        if isinstance(gender_dict, dict):
            # 키 정규화(혹시 소문자로 오는 경우 대비)
            norm = {}
            ...
            merged['_ema_gender'] = ema_g
            man = ema_g.get('Man') or 0.0
            wom = ema_g.get('Woman') or 0.0
            merged['dominant_gender'] = 'Man' if man >= wom else 'Woman'
            merged['gender'] = norm
        else:
            dg = ag_result.get('dominant_gender') if isinstance(ag_result, dict) else None
            if isinstance(dg, str):
                merged['dominant_gender'] = dg
            else:
                merged.setdefault('dominant_gender', prev.get('dominant_gender', 'Man'))

        # UI KeyError 방지(최소 키 보장)
        merged.setdefault('dominant_gender', 'Man')
        merged.setdefault('age', 0)
        merged.setdefault('dominant_emotion', 'neutral')
...
while cap.isOpened():
    ...
    if result.face_landmarks:
        for idx, landmarks in enumerate(result.face_landmarks):
            ...
            gaze, _dbg = get_detailed_gaze(landmarks)
            ...
            if faces_status[idx]['data']:
                ...
                gender = "M" if d.get('dominant_gender') == 'Man' else "W"
                info = f"ID:{idx+1} | {gender} | {d.get('age', 0)}s"

                # beforeme.py 방식: smoothed_emotion 있으면 그걸 표시
                if 'smoothed_emotion' in d:
                    display_emotion = d['smoothed_emotion']
                    display_conf = d.get('smoothed_confidence', 0.0)
                    emo = f"Emotion: {display_emotion.upper()} ({display_conf:.0f}%)"
                else:
                    emo = f"Emotion: {d.get('dominant_emotion','neutral').upper()}"
                ...
                cv2.putText(frame, f"ID:{idx+1} Initializing...", (25, panel_y + 30),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (150, 150, 150), 1)

            cv2.putText(frame, f"Gaze: {gaze}", (25, panel_y + 65),
...
# :contentReference[oaicite:0]{index=0}  (beforeme.py: emotion smoothing + opencv backend pipeline)
# :contentReference[oaicite:1]{index=1}  (main.py: age/gender preprocess + skip backend + EMA)
